...
class COCOModel(ff.FastFlowModel):
    def __init__(self):
        super().__init__()
...
class COCOApp(App):
    ...
    def create_model(self):
        ...
        model.compile(optimizer="adam", loss=self.dummy_loss, run_eagerly=False)
        return model

    def create_dataset(self, num_parallel):
        ann_file = DATASET_LOC + "/annotations/instances_val2017.json"
        ...
        for img in ann_json["images"]:
            ...
            sample = (
                DATASET_LOC + "/val2017/" + img["file_name"],
                labels,
                boxes,
            )
        ...
        ds = ds.map(read_image, num_parallel_calls=num_parallel)
        ds = ds.map(
            distorted_bounding_box_crop, num_parallel_calls=num_parallel
        )
        ds = ds.map(resize_image, num_parallel_calls=num_parallel)
        ds = ds.map(random_flip, num_parallel_calls=num_parallel)
        ds = ds.map(distort, num_parallel_calls=num_parallel)
        ds = ds.map(normalize, num_parallel_calls=num_parallel)
        ds = ds.batch(1)
