...
class Tokenizer:
    ...
    def __call__(self, x):
        ...
        return {"text": self.tokenizer.tokenize(data)}


def _truncate(x):
    dim = tf.shape(x["text"])[0]
    slice_size = tf.minimum(dim, 254)
    return {"text": tf.slice(x["text"], [0], [slice_size])}


class Embedding:
    def __init__(self):
        self.embedding = tf.Variable(
            tf.random.uniform([50001, 768], -1.0, 1.0)
        )
