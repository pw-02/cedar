...
def _get_profiler(dataset_file: str, dataset_func: str, spec: TFEvalSpec):
    try:
...
def create_spec(args: argparse.Namespace) -> TFEvalSpec:
    if args.num_parallel_calls is None:
        num_parallel_calls = None
    elif args.num_parallel_calls == -1:
        num_parallel_calls = tf.data.AUTOTUNE
    else:
        num_parallel_calls = args.num_parallel_calls

    logger.info("Using {} parallel calls".format(num_parallel_calls))
    return TFEvalSpec(
        args.batch_size,
        num_parallel_calls,
        args.num_epochs,
...
def main():
    ...
    parser.add_argument(
        "--num_parallel_calls",
        type=int,
        help="Number of parallel calls to make. If -1, use autotune. If unset"
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="If service_addr is set, use tf.data.service with the "
        "dispatcher at the provided address. If not set, execute locally.",
    )
    parser.add_argument(
        "--read_from_remote",
        action="store_true",
